...
class Node:
    ...
    def InfromationGain(self, len_parent, dict):
        sum=0
        for i in dict.keys():
            var=np.log2(len(i))
            sum+=var
        child_entropy=sum
        parent_entropy=np.log2(len_parent)
    ...
    def process_node( self, all_words, my_words_idx, history, verbose ):
        ...
        for i in range(len(query_array)):
            split_dict={}
            for idx in my_words_idx:
                mask = self.reveal( all_words[ idx ], query_array[i] )
                if mask not in split_dict:
                    split_dict[ mask ] = []
            
                split_dict[ mask ].append( idx )

            if max_info_gain<self.InfromationGain(len(my_words_idx),split_dict):
                max_info_gain=self.InfromationGain(len(my_words_idx),split_dict)
                best_query=i 
...
# def best_split(self, all_words, my_words_idx, history, verbose): 
    # For the root we do not ask any query -- Melbot simply gives us the length of the secret word 
    # if len(history) == 0: 
    #     query_idx = -1 
    #     query = ""
    ...
    #         if max_info_gain<InfromationGain(len(my_words_idx),split_dict):
    #             max_info_gain=InfromationGain(len(my_words_idx),split_dict)
    #             best_query=i 
   
   
    # def InfromationGain(self, len_parent, dict):
    #     child_entropy=np.sum(np.log((len(i) for i in dict.items())))
    #     parent_entropy=np.log(len_parent)
    
    #     return parent_entropy-child_entropy
    
    # return (best_query, split_dict)
